...
# Fetched once per REV and reused: .rev records what src/ holds. The debian/
# overlay below is re-applied every time regardless, which is what keeps the
# version rewrite idempotent.
if [ -f "${DIR}/pin" ]; then
    ...
    if [ -f "${DIR}/debian/changelog" ] && [ "$(cat "${WORK}/.rev" 2>/dev/null)" = "${REV}" ]; then
        ...
        # The pinned commit, not the branch tip. GitHub serves a sha directly,
        # so this stays a shallow fetch however far the branch has moved on.
...
# Our debian/ is laid over whatever upstream ships: just the changelog for a
# source that carries its own packaging, the whole thing for one that does not.
...
# ~<suite> keeps the same committed version distinct across suites. The
...
# A package here may build-depend on another one here, so whatever this suite
# has already built is offered to the container as a local repo. Only this
# run's debs, never the published archive: a commit that moves mpp and
# rebuilds gstreamer must link the new gstreamer against the new mpp.
LOCAL=()
...
# The chown back at the end is what lets a later run rm -rf its own work/.
docker run --rm -v "${PWD}/${WORK}:/w" -w /w/src "${LOCAL[@]}" \
    ...
    "${IMAGE}" bash -euc '
        ...
        # Copied out of the read-only mount: apt wants the Packages index to
        # sit beside the debs it indexes.
        if [ -d /deb ]; then
...
# Debug symbols are dropped on purpose: several times the size of what they
# describe, against a 1 GB budget for the whole Pages site. Matched on the
# -dbgsym_ name rather than on the extension, which is not the same on every
# suite -- dpkg-deb writes .ddeb on Ubuntu and .deb on Debian.
#
# mk-build-deps -r already removes the -build-deps metapackage, so it is not
# normally here to be moved; excluded anyway rather than resting on that flag.
mkdir -p "out/${SUITE}"
for deb in "${WORK}"/*.deb; do
    case "${deb}" in
        *-build-deps_*.deb|*-dbgsym_*.deb)
            echo "==> skipping $(basename "${deb}")"; continue ;;
    esac
    mv "${deb}" "out/${SUITE}/"
done
rm -f "${WORK}"/*-build-deps_*.deb "${WORK}"/*-dbgsym_*.deb
